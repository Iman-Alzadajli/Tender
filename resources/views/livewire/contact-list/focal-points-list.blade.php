<div>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            ...
            <div class="border-top pt-3 mt-3">
                <div class="row g-3">
                    <div class="col-md-6"><label for="clientFilter" class="form-label">Client Name</label><select id="clientFilter" wire:model.live="clientFilter" class="form-select">
                            <option value="">All Clients</option>@foreach($clients as $client)<option value="{{ $client }}">{{ $client }}</option>@endforeach
                        </select></div>
                    <div class="col-md-6"><label for="clientTypeFilter" class="form-label">Client Type</label><select id="clientTypeFilter" wire:model.live="clientTypeFilter" class="form-select">
                            <option value="">All Client Types</option>@foreach($clientTypes as $type)<option value="{{ $type }}">{{ $type }}</option>@endforeach
                        </select></div>
                </div>
            </div>
            @endif
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    ...
                    <tr wire:key="{{ $fp->id }}-{{ $fp->phone }}-{{ $fp->email }}">
                        {{-- ✅✅✅ ترتيب البيانات حسب المطلوب --}}
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4">No focal points found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="modal-content">
                <form wire:submit.prevent="updateFocalPoint">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Focal Point</h5><button type="button" class="btn-close" wire:click="closeModal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="alert alert-info">
                            <small><i class="bi bi-info-circle"></i> This update will apply to all duplicate contacts with the same phone and email.</small>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3"><label for="fp_phone" class="form-label">Phone</label><input type="text" id="fp_phone" wire:model="fp_phone" class="form-control @error('fp_phone') is-invalid @enderror">@error('fp_phone') <span class="invalid-feedback">{{ $message }}</span> @enderror</div>
                            <div class="col-md-6 mb-3"><label for="fp_email" class="form-label">Email</label><input type="email" id="fp_email" wire:model="fp_email" class="form-control @error('fp_email') is-invalid @enderror">@error('fp_email') <span class="invalid-feedback">{{ $message }}</span> @enderror</div>
                        </div>
    ...
    <div class="modal fade show" style="display: block; background-color: rgba(0,0,0,0.5);" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-danger">Confirm Deletion</h5><button type="button" class="btn-close" wire:click="$set('showDeleteModal', false)"></button>
                </div>
                <div class="modal-body">
                    <p>Are you sure you want to delete this focal point?</p>
                    <p class="fw-bold text-center fs-5 my-3">{{ $deletingFocalPointName }}</p>
