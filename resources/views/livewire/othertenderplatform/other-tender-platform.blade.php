        ...
        <div class="card-body">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
                <div class="d-flex gap-2">
                    <button wire:click="prepareModal('add')" class="btn btn-primary" @cannot('other-tenders.create') disabled @endcannot>
                        <i class="bi bi-plus-lg me-2"></i>Add Tender
                    </button>
                    <button wire:click="exportPdf" class="btn btn-danger" @cannot('other-tenders.export') disabled @endcannot>
                    ...
                    <button wire:click="exportSimpleExcel" class="btn btn-success" @cannot('other-tenders.export') disabled @endcannot>
            ...
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group">
                                <button wire:click="prepareModal('view', {{ $tender->id }})" class="btn btn-sm btn-outline-secondary" title="View" @cannot('other-tenders.view') disabled @endcannot><i class="bi bi-eye"></i></button>
                                <button wire:click="prepareModal('edit', {{ $tender->id }})" class="btn btn-sm btn-outline-primary" title="Edit" @cannot('other-tenders.edit') disabled @endcannot><i class="bi bi-pencil"></i></button>
                                <button wire:click="deleteTender({{ $tender->id }})" wire:confirm="Are you sure?" class="btn btn-sm btn-outline-danger" title="Delete" @cannot('other-tenders.delete') disabled @endcannot>
                ...
                <form wire:submit.prevent="save">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            @if($mode != 'view')
                            <button wire:click.prevent="addFocalPoint" type="button" class="btn btn-sm btn-outline-primary" @cannot('other-tenders.manage-focal-points') disabled @endcannot>
                                <i class="bi bi-plus"></i> Add Person</button>@endif
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            @if($mode != 'view')
                            <button wire:click.prevent="addPartnership" type="button" class="btn btn-sm btn-outline-primary @if(!$has_third_party) disabled @endif" @cannot('other-tenders.manage-partnerships') disabled @endcannot>
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex align-items-center pt-1">
                                    <div class="form-check me-4">
                                        <input class="form-check-input" type="radio" wire:model.live="has_third_party" value="1" id="thirdPartyYes" @if($mode=='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) disabled @endif>
                                        <label class="form-check-label" for="thirdPartyYes">Yes</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" wire:model.live="has_third_party" value="0" id="thirdPartyNo" @if($mode=='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) disabled @endif>
                                ...
                                <div class="card mb-3">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            @if($mode != 'view')
                                            <button wire:click.prevent="removePartnership({{ $index }})" type="button" class="btn-close" title="Remove Partner" @cannot('other-tenders.manage-partnerships') disabled @endcannot></button>
                                            @endif
                                        </div>
                                        <div class="row g-3">
                                            <div class="col-md-6"><label class="form-label">Company Name <span class="text-danger">*</span></label><input type="text" wire:model="partnerships.{{ $index }}.company_name" class="form-control @error('partnerships.'.$index.'.company_name') is-invalid @enderror" @if($mode==='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) readonly @endif>@error('partnerships.'.$index.'.company_name') <div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                            <div class="col-md-6"><label class="form-label">Person Name <span class="text-danger">*</span></label><input type="text" wire:model="partnerships.{{ $index }}.person_name" class="form-control @error('partnerships.'.$index.'.person_name') is-invalid @enderror" @if($mode==='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) readonly @endif>@error('partnerships.'.$index.'.person_name')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                            <div class="col-md-6"><label class="form-label">Phone <span class="text-danger">*</span></label><input type="text" wire:model="partnerships.{{ $index }}.phone" class="form-control @error('partnerships.'.$index.'.phone') is-invalid @enderror" @if($mode==='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) readonly @endif>@error('partnerships.'.$index.'.phone')<div class="invalid-feedback">{{ $messages['partnerships.*.phone.regex'] ?? $message }}</div>@enderror</div>
                                            <div class="col-md-6"><label class="form-label">Email <span class="text-danger">*</span></label><input type="email" wire:model="partnerships.{{ $index }}.email" class="form-control @error('partnerships.'.$index.'.email') is-invalid @enderror" @if($mode==='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) readonly @endif>@error('partnerships.'.$index.'.email')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                            <div class="col-12"><label class="form-label">Collaboration Details</label><textarea wire:model="partnerships.{{ $index }}.details" class="form-control" rows="2" @if($mode==='view' || Auth::user()->cannot('other-tenders.manage-partnerships')) readonly @endif></textarea></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
